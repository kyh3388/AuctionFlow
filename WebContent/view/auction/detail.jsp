		...
		%>
			...
			<section class="detail-top">
				...
				<div class="detail-information">
					...
					<p class="detail-bid-guide" id="detailBidGuide">
						...
						%>
							입찰 단위는
							<%= priceFormat.format(bidUnit) %>원입니다.
						<%
							}
						%>
					</p>
			...
			<div id="detailCancelContainer">
				...
				%>
					...
					<section
						class="detail-cancel-section"
						id="detailCancelSection">
		...
		<div class="bid-dialog-inner">
			...
			<div class="bid-dialog-body">
				...
				<div class="bid-dialog-left">
					...
					<section class="bid-flow-panel" id="bidFlowPanel" aria-label="동시 입찰 처리 시각화">
						...
						<div class="bid-demo-count-control">
							...
							<input
								type="number"
								id="bidDemoRequestCountInput"
								class="bid-demo-count-input"
								value="100"
								min="1"
								step="1"
								inputmode="numeric"
				...
				<div class="bid-dialog-right">
					...
					<form
						action="${pageContext.request.contextPath}/api/auctionFlow/auction/bid"
						method="post"
						class="bid-form"
						id="bidForm">
						...
						<button
							...
							id="bidSubmitButton">
							KRW <%= priceFormat.format(nextBidPrice) %> 입찰하기
						</button>
					</form>
...
(function () {
	...
	function initializeAuctionDetail() {
		...
		const bidRequestIdInput =
			document.getElementById(
				"bidRequestId"
			);

		const bidRemainingTime =
			document.getElementById(
				"bidRemainingTime"
			);
		...
		const loginUrl =
			...
			+ "/api/auctionFlow/member/loginForm";
			...
			function (thumbnailButton) {
				...
				thumbnailButton.addEventListener(
					...
					function () {
						...
						const imageSrc =
							thumbnailButton.dataset.imageSrc;
		...
		function parseDatabaseDatetime(
			datetimeText
		) {
			...
			const matchedDatetime =
				normalizedDatetime.match(
		...
		function updateRemainingTime() {
			...
			const endDatetime =
				parseDatabaseDatetime(
					bidDialog.dataset.endDatetime
				);

			if (!endDatetime) {

				bidRemainingTime.textContent =
					"종료 시간 확인 불가";
		...
		window.createAuctionBidRequestId =
			createBidRequestId;
		...
		if (bidOpenButton) {
			...
			bidOpenButton.addEventListener(
				"click",
				function () {
					...
					if (
						typeof bidDialog.showModal
							=== "function"
					) {
						bidDialog.showModal();

					} else {
						bidDialog.setAttribute(
							"open",
							"open"
						);
					}
					...
					if (
						typeof window.refreshRealtimeAuction
							=== "function"
					) {
...
document.addEventListener(
	...
	function () {
		...
		/*
		 * WebSocket 이벤트가 누락되거나 HTTP 요청과 겹쳐도
		 * 화면이 반드시 DB 최신값을 따라가도록 짧은 주기로 보정한다.
		 */
		const REALTIME_SYNC_INTERVAL_MS =
			500;

		const AUCTION_START_PRICE =
			<%= auctionStartPrice %>;

		const AUCTION_BID_UNIT =
			<%= bidUnit %>;

		let appliedBidCount =
			<%= auctionBidCount %>;

		let appliedCurrentPrice =
			<%= auctionCurrentPrice %>;

		let latestRealtimeSnapshot =
			null;

		let lastQueuedRealtimeSignature =
			null;

		let realtimeAnimationRunning =
			false;

		let realtimeRefreshRunning =
			false;

		let realtimeRefreshRequested =
			false;

		let realtimePollTimerId =
			null;
		...
		const bidHistorySummaryPrice =
			document.getElementById(
				"bidHistorySummaryPrice"
			);

		const bidHistorySummaryCount =
			document.getElementById(
				"bidHistorySummaryCount"
			);
		...
		const bidDemoBatchButton =
			document.getElementById(
				"bidDemoBatchButton"
			);

		const bidDemoRequestCountInput =
			document.getElementById(
				"bidDemoRequestCountInput"
			);
		...
		const bidFlowTokenLayer =
			document.getElementById(
				"bidFlowTokenLayer"
			);

		const bidFlowPendingCount =
			document.getElementById(
				"bidFlowPendingCount"
			);

		const bidFlowSuccessCount =
			document.getElementById(
				"bidFlowSuccessCount"
			);

		const bidFlowRejectedCount =
			document.getElementById(
				"bidFlowRejectedCount"
			);
		...
		async function runSingleBidCountRoll(
			...
			viewportElement.className =
				"bid-count-roll-viewport";

			viewportElement.appendChild(
				trackElement
			);
			...
			try {
				...
				console.debug(
					"[DETAIL BID COUNT ROLL CANCELED]",
					error
				);
			}

			setFormattedBidCount(
				element,
				endCount,
				prefix,
				suffix
		...
		function getBidKeyFromElement(
			itemElement
		) {

			if (
				itemElement.dataset.bidNo
			) {
				return "BID_NO|"
					+ itemElement.dataset.bidNo;
			}
		...
		const knownBidKeySet =
			new Set();

		let bidHistoryAnimationQueue =
			Promise.resolve();

		let bidHistoryUpdateVersion =
			0;
		...
		function initializeKnownBidKeys() {
			...
			const currentItems =
				bidHistoryList.querySelectorAll(
					".bid-history-item"
				);
		...
		function renderBidHistorySnapshot(
			bidList
		) {
		...
		async function animateNewBidHistoryItem(
			bid,
			shouldAnimate
		) {
			...
			flyingElement.classList.add(
				"bid-history-flying-item"
			);

			flyingElement.style.top =
				Math.max(
					8,
					bidHistoryList.clientHeight
						- itemHeight
				)
				+ "px";
		...
		function queueBidHistoryUpdate(
			bidList
		) {
			...
			const normalizedBidList =
				normalizeBidList(
					bidList
				);

			const updateVersion =
				++bidHistoryUpdateVersion;

			const newBidList =
				normalizedBidList.filter(
					function (bid) {

						return !knownBidKeySet.has(
							getBidKeyFromData(
								bid
							)
						);
							...
							bidHistoryAnimationQueue
								...
								.then(
									function () {
										...
										return animateNewBidHistoryItem(
											bid,
											shouldAnimate
										);
									}
		...
		function normalizeRealtimeSnapshot(
			realtimeData
		) {
			...
			if (
				String(
					realtimeData.auctionNo
				)
				!== String(auctionNo)
			) {

				console.warn(
					"[AUCTION REALTIME] 다른 경매 데이터",
					realtimeData
				);
			...
			const bidCount =
				Number(
					realtimeData.bidCount
				);

			const nextBidPrice =
				realtimeData.nextBidPrice
					== null
					? null
					: Number(
						realtimeData.nextBidPrice
			...
			const demoState =
			    realtimeData.demoState == null
			        ? "READY"
			        : String(
			            realtimeData.demoState
			        )
			            .trim()
			            .toUpperCase();

			const demoRemainingMillis =
			    Number(
			        realtimeData.demoRemainingMillis
			    );
		...
		function updateBidActionFromSnapshot(
			snapshot
		) {
		...
		function calculatePriceForBidCount(
			bidCount,
			targetSnapshot
		) {
		...
		async function animateRealtimeStep(
			nextPrice,
			nextBidCount
		) {
		...
		function applyBidHistorySnapshot(
			snapshot
		) {

			try {

				queueBidHistoryUpdate(
					snapshot.bidList
				);
				...
				console.error(
					"[AUCTION BID HISTORY UPDATE FAILED]",
					error
				);
			}
		}
			...
			try {
				...
				while (latestRealtimeSnapshot) {
					...
					if (
						...
						const nextPrice =
							calculatePriceForBidCount(
								nextBidCount,
								snapshot
							);

						await animateRealtimeStep(
							nextPrice,
							nextBidCount
						);
					...
					updateBidActionFromSnapshot(
						snapshot
					);

					applyBidHistorySnapshot(
						snapshot
					);

					console.log(
						"[AUCTION REALTIME APPLIED]",
						snapshot
					);
				...
				console.error(
					"[AUCTION REALTIME ANIMATION FAILED]",
					error
				);
		...
		function applyRealtimeAuctionData(
			...
			syncBidDemoUiFromSnapshot(
				    snapshot
				);

			/*
			 * 실제 입찰 버튼에는 애니메이션 완료 전이라도
			 * 서버가 확정한 최신 다음 입찰가를 즉시 넣는다.
			 */
			updateBidActionFromSnapshot(
				snapshot
			);
			...
			const latestBid =
				snapshot.bidList.length > 0
					? snapshot.bidList[0]
					: null;

			const realtimeSignature =
				String(snapshot.bidCount)
				+ "|"
				+ String(snapshot.currentPrice)
				+ "|"
				+ String(
					latestBid
						&& latestBid.bidNo != null
							? latestBid.bidNo
							: ""
				);
			...
			if (
				latestRealtimeSnapshot
				&& snapshot.bidCount
					< latestRealtimeSnapshot.bidCount
			) {
				return;
			}
			...
			try {
				...
				while (realtimeRefreshRequested) {
					...
					try {
						...
						console.log(
							"[AUCTION REALTIME REQUEST] REASON="
							+ (reason || "UNKNOWN"),
							realtimeUrl
						);
		...
		let activeBidDemoSpawnPromise =
			Promise.resolve();

		/*
		 * 동시 시연 UI 상태.
		 * READY / STARTING / RUNNING / COOLDOWN
		 */
		...
		/*
		 * SVG는 viewBox="0 0 300 132" 좌표계를 사용하고,
		 * 공은 HTML span 요소이므로 SVG 논리 좌표를 실제 px 좌표로 변환한다.
		 */
		...
		function clampBidFlowLogicalY(logicalX, logicalY, padding) {
			...
			const bounds =
				getBidFlowFunnelBounds(logicalX);
			...
			return Math.max(
				bounds.top + safePadding,
				Math.min(
					bounds.bottom - safePadding,
					logicalY
				)
			);
		}
		...
		/*
		 * 왼쪽 출발선.
		 * 위/아래 선 자체가 아니라 그 사이 공간의 랜덤 위치에서 출발한다.
		 */
		...
		function getBidFlowMiddleLogicalPosition(entryLogicalY) {
			...
			let logicalY =
				66
				+ (entryLogicalY - 66) * 0.52
				+ (Math.random() * 8 - 4);

			logicalY =
				clampBidFlowLogicalY(
					logicalX,
					logicalY,
					4
				);
		...
		function updateBidFlowTokenDensity() {
			...
			const tokenSize =
				getBidFlowTokenSize(
					bidFlowTokenMap.size
				);

			bidFlowPanel.style.setProperty(
				"--bid-flow-token-size",
				tokenSize + "px"
			);
		}
		...
		function updateBidFlowCounters() {
			...
			const pendingCount =
				Math.max(
					0,
					bidFlowAllRequestIds.size
					- bidFlowResolvedRequestIds.size
				);

			if (bidFlowSuccessCount) {
				bidFlowSuccessCount.textContent =
					String(bidFlowSuccessRequestIds.size);
			}

			if (bidFlowRejectedCount) {
				bidFlowRejectedCount.textContent =
					String(bidFlowRejectedRequestIds.size);
			}

			if (bidFlowPendingCount) {
				bidFlowPendingCount.textContent =
					String(pendingCount);
			}
		}
		...
		function clearBidFlowTokenExpiry(requestId) {
			...
			const timerId =
				bidFlowTokenExpiryTimerMap.get(requestId);
		...
		function resetBidFlowVisualization() {
			...
			bidFlowTokenExpiryTimerMap.forEach(
				function (timerId) {
					window.clearTimeout(timerId);
				}
			);
		...
		function getBidFlowPileLogicalPosition(tokenElement, pileIndex) {
			...
			const tokenSizeLogical =
				(tokenSizePx / Math.max(stageWidth, 1))
				* 300;

			const horizontalSpacing =
				Math.max(
					4.2,
					tokenSizeLogical + 0.9
				);

			const verticalSpacing =
				Math.max(
					4.0,
					tokenSizeLogical + 0.7
				);
		...
		function getBidFlowBurstLaunchDelay() {
			...
			return Math.min(
				bidFlowBurstLaunchIndex
				* BID_FLOW_SPAWN_INTERVAL_MS,
				2200
			);
		}
		...
		function enqueueBidFlowRequest(requestId, deferRender) {
			...
			tokenElement.dataset.requestId =
				requestId;

			tokenElement.dataset.result =
				"PENDING";

			/* 실제 발사 전까지 왼쪽 위에 보이지 않도록 숨긴다. */
			tokenElement.style.visibility =
				"hidden";

			bidFlowTokenLayer.appendChild(
				tokenElement
			);
			...
			const visualState =
				createBidFlowVisualState(requestId);
			...
			if (deferRender !== true) {
				...
				const expiryTimerId =
					window.setTimeout(
						function () {
							...
							if (
								bidFlowResolvedRequestIds.has(requestId)
							) {
								return;
							}

							resolveBidFlowRequest(
								requestId,
								"REJECTED",
								"STALE_EVENT"
							);
				...
				bidFlowTokenExpiryTimerMap.set(
					requestId,
					expiryTimerId
				);
			}
			...
			const timerId =
				window.setTimeout(
					function () {
						...
						startBidFlowTokenFlight(
							requestId,
							visualState.pileIndex,
							bidFlowGeneration
						);
					},
					launchDelay
		...
		/*
		 * 공 1개의 기본 이동:
		 * 왼쪽 랜덤 출발 → 깔대기 중간 → LOCK 앞의 자기 pile 자리.
		 */
		async function startBidFlowTokenFlight(
			requestId,
			pileIndex,
			animationGeneration
		) {
			...
			const visualState =
				bidFlowVisualStateMap.get(requestId);

			if (
				!tokenElement
				|| !visualState
				|| animationGeneration !== bidFlowGeneration
			) {
				return;
			}

			const entryLogicalPosition =
				getBidFlowEntryLogicalPosition();

			visualState.entryLogicalY =
				entryLogicalPosition.y;

			const entryPosition =
				getBidFlowScreenPosition(
					tokenElement,
					entryLogicalPosition.x,
					entryLogicalPosition.y
				);
			...
			const middleLogicalPosition =
				getBidFlowMiddleLogicalPosition(
					entryLogicalPosition.y
				);
			...
			const middlePosition =
				getBidFlowScreenPosition(
					tokenElement,
					middleLogicalPosition.x,
					middleLogicalPosition.y
				);
			...
			await waitForBidFlowAnimation(
				BID_FLOW_FIRST_MOVE_MS
			);
			...
			const pileLogicalPosition =
				getBidFlowPileLogicalPosition(
					tokenElement,
					pileIndex
				);
			...
			const pilePosition =
				getBidFlowScreenPosition(
					tokenElement,
					pileLogicalPosition.x,
					pileLogicalPosition.y
				);
		...
		function applyBidFlowVisualResult(requestId) {
			...
			tokenElement.dataset.reason =
				visualState.reason
				|| "REJECTED";

			bidFlowRejectedVisibleRequestIds.add(
				requestId
			);
		...
		async function animateSuccessfulBidFlow(
			requestId,
			animationGeneration
		) {
			...
			/*
			 * pile의 가장 앞 공이 아니더라도
			 * 실제 SUCCESS requestId의 공을 LOCK 바로 앞으로 끌어온다.
			 */
		...
		async function playNextBidFlowSuccess() {
			...
			const requestId =
				bidFlowSuccessQueue.shift();

			bidFlowSuccessAnimatingGeneration =
				animationGeneration;

			try {
				await animateSuccessfulBidFlow(
					requestId,
					animationGeneration
				);
			} catch (error) {
				console.error(
					"[BID FLOW SUCCESS ANIMATION FAILED]",
					error
				);
			} finally {
		...
		function removeRejectedBidFlowTokens() {
			...
			const rejectedRequestIds =
				Array.from(
					bidFlowRejectedVisibleRequestIds
				);
		...
		function forceCleanupRemainingBidFlowTokens() {
			...
			if (
				bidFlowRejectedVisibleRequestIds.size > 0
			) {
				removeRejectedBidFlowTokens();
			}
		}
		...
		function scheduleBidFlowForceCleanup() {
			...
			if (bidFlowForceCleanupTimer !== null) {
				window.clearTimeout(
					bidFlowForceCleanupTimer
				);
			}

			bidFlowForceCleanupTimer =
				window.setTimeout(
					forceCleanupRemainingBidFlowTokens,
					BID_FLOW_FORCE_CLEANUP_MS
				);
		}
		...
		function promoteSuccessfulBidFlowTokenToFront(requestId) {
			...
			const originalSuccessPileIndex =
				successState.pileIndex;

			successState.pileIndex = 0;
			frontState.pileIndex =
				originalSuccessPileIndex;
		}

		function resolveBidFlowRequest(
			requestId,
			result,
			reason
		) {
		...
		/* =========================
		   ...
		   function createBidDemoVisualRequestId(
					demoId,
					index
				) {
				...
				/*
				 * 서버가 BID_DEMO_STARTED를 보내면
				 * 모든 브라우저가 자기 화면에서
				 * 똑같은 개수의 공을 생성한다.
				 */
				function startBidDemoVisualization(
					demoId,
					requestCount
				) {
					...
					/*
					 * 같은 START 이벤트가 중복으로 와도
					 * 다시 생성하지 않는다.
					 */
					if (
						activeBidDemoId === demoId
						&& activeBidDemoRequestIds.length
							=== normalizedRequestCount
					) {
						return;
					}
					...
					for (
						let index = 0;
						index < normalizedRequestCount;
						index++
					) {

						const visualRequestId =
							createBidDemoVisualRequestId(
								demoId,
								index
							);

						activeBidDemoRequestIds.push(
							visualRequestId
						);
					...
					activeBidDemoSpawnPromise =
						playBidFlowDemoSpawnSequence(
							requestDefinitions,
							animationGeneration
						).catch(
							function (error) {
				...
				/*
				 * 서버가 BID_DEMO_FINISHED를 보내면
				 * 모든 브라우저에서 동일한 공을
				 * 동일한 SUCCESS / REJECTED 수로 처리한다.
				 */
				function finishBidDemoVisualization(
					demoId,
					successCount,
					rejectedCount
				) {
					...
					let normalizedSuccessCount =
						Number(successCount);

					let normalizedRejectedCount =
					...
					activeBidDemoRequestIds.forEach(
						function (
							requestId,
							index
						) {
				 ...
				 */
				async function sendBidDemoControlRequest(
					requestUrl,
					requestValues
				) {
					...
					if (
						...
						requestError.remainingMillis =
							resultData
								? Number(
									resultData.remainingMillis
								)
								: 0;
		...
		function readBidDemoRequestCount() {
			...
			const requestCountText =
				bidDemoRequestCountInput.value.trim();
		...
		function lockBidDemoControls(
			buttonText
		) {

			if (bidDemoBatchButton) {

				bidDemoBatchButton.disabled =
					true;

				if (buttonText) {

					bidDemoBatchButton.textContent =
						buttonText;
				}
			}
			...
			if (
				...
				bidDemoRequestCountInput.disabled =
					true;
			}
		}
		...
		function setBidDemoRunningUi() {
			...
			/*
			 * FINISH 이벤트가 오지 않는 비정상 상황에서도
			 * 서버의 RUNNING 만료시간과 맞춰 영구 잠금을 방지한다.
			 */
			bidDemoRunningTimeoutId =
				window.setTimeout(
					function () {
		...
		function syncBidDemoUiFromSnapshot(
			    snapshot
			) {
			    ...
			    if (
			        ...
			        /*
			         * 이미 WebSocket으로 RUNNING 상태라면
			         * 500ms polling마다 타이머를 다시 만들 필요가 없다.
			         */
			        if (
			            bidDemoUiState
			            !== "RUNNING"
			    ...
			    if (
			        ...
			        if (
			            snapshot.demoRemainingMillis
			            > 0
			        ) {

			            startBidDemoCooldown(
			                snapshot.demoRemainingMillis
			            );
			        }
		 ...
		 * HTTP 요청의 동시성과는 별개다.
		 */
		async function playBidFlowDemoSpawnSequence(
			requestDefinitions,
			animationGeneration
		) {
			...
			for (
				let index = 0;
				index < requestDefinitions.length;
				index++
			) {
		...
		if (bidDemoBatchButton) {
			...
			bidDemoBatchButton.addEventListener(
				"click",
				async function () {
					...
					const demoRequestCount =
						readBidDemoRequestCount();

					if (
						demoRequestCount === null
					) {

						window.alert(
							"시연 요청 수는 1 이상의 정수로 입력해 주세요."
						);
					...
					if (
						typeof window
							.createAuctionBidRequestId
						!== "function"
					) {

						console.error(
							"[BID DEMO] 요청 식별번호 생성 함수 없음"
						);

						window.alert(
							"입찰 요청 식별번호 생성 기능을 불러오지 못했습니다."
						);
					...
					async function loadBidDemoRealtimeSnapshotWithRetry(
						maxAttempts,
						delayMilliseconds
					) {
					...
					try {
						 ...
						 */
						const demoStartRealtime =
							await loadBidDemoRealtimeSnapshotWithRetry(
								5,
								300
							);

						const demoStartBidCount =
							demoStartRealtime
								.snapshot
								.bidCount;
						 ...
						 */
						await sendBidDemoControlRequest(
							bidDemoStartUrl,
							{
								A_NO:
									auctionNo,
						...
						const demoEndRealtime =
							await loadBidDemoRealtimeSnapshotWithRetry(
								10,
								300
							);

						const demoEndBidCount =
							demoEndRealtime
								.snapshot
								.bidCount;
						...
						applyRealtimeAuctionData(
							demoEndRealtime.rawData
						);

						const failedHttpCount =
						...
						try {
							...
							finishBroadcastSucceeded =
								false;

							console.error(
								"[BID DEMO FINISH BROADCAST FAILED]",
								error
							);
						}

						finishBidDemoVisualization(
							demoId,
							successCount,
							rejectedCount
						);
						...
						if (
							...
							console.warn(
								"[BID DEMO] FINISH WebSocket 동기화 실패"
							);
						}
						...
						if (
							...
							window.alert(
								"현재 이 경매에서 다른 동시 요청 시연이 진행 중입니다."
							);
							...
							startBidDemoCooldown(
								error.remainingMillis
							);

							window.alert(
								"이 경매는 방금 시연이 종료되어 잠시 후 다시 시연할 수 있습니다."
							);
						...
						/*
						 * 시연 버튼/요청 수 입력창은
						 * READY/RUNNING/COOLDOWN 상태 함수가 관리한다.
						 */
						if (
							bidSubmitButton
						) {
					...
					runBidFlowSafely(
						...
						function () {
							...
							finishBidDemoVisualization(
								messageValues.DEMO_ID,
								Number(
									messageValues.SUCCESS
								),
								Number(
									messageValues.REJECTED
								)
							);
						}
					);

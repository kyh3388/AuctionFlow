...
<main class="main">
	...
	<section class="find-password">
		...
		<div class="find-password-inner">
			...
			<div class="guide-box">
				<p class="guide-text">- 비밀번호의 경우, 암호화 저장되어 기존 비밀번호를 확인할 수 없습니다.</p>
				<p class="guide-text highlight">- 회원정보가 일치하면 임시 비밀번호를 발급해 드립니다.</p>
				<p class="guide-text">- 발급된 임시 비밀번호로 로그인한 뒤, 마이페이지에서 반드시 비밀번호를 변경해 주세요.</p>
			</div>

			<form action="${pageContext.request.contextPath}/api/auctionFlow/member/findPassword" method="post" class="find-password-form" id="findPasswordForm">
				<div class="find-type-area">
					<label class="find-type-label">
						<input type="radio" name="FIND_TYPE" id="findTypePhone" value="PHONE" ${FIND_TYPE == 'EMAIL' ? '' : 'checked'}>
				...
				<div class="form-row ${FIND_TYPE == 'EMAIL' ? '' : 'is-hidden'}" id="emailRow">
					...
					<input type="email" id="memberEmail" name="FIND_EMAIL" class="input" value="${FIND_EMAIL}" placeholder="example@email.com" ${FIND_TYPE == 'EMAIL' ? '' : 'disabled'}>
				</div>

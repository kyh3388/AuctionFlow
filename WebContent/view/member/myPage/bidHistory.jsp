...
<body class="mypage-body bid-history-page" data-context-path="${pageContext.request.contextPath}">
				...
				<div class="bid-history-table-wrap">
					<table class="bid-history-table">
						...
						%>
							...
							<tr class="<%= "ONGOING".equals(auctionStatus) ? "mypage-auction-realtime-item" : "" %>"
								data-auction-no="<%= auctionNo %>" data-auction-status="<%= escapeHtml(auctionStatus) %>"
								...
								<td class="bid-history-product-cell">
									<a href="${pageContext.request.contextPath}/api/auctionFlow/auction/detail?A_NO=<%= auctionNo %>" class="bid-history-product-link">
										<div class="bid-history-image-box">
											...
											%>
												<img src="${pageContext.request.contextPath}/api/auctionFlow/auction/image?IMG_STORED_NAME=<%= escapeHtml(imageStoredName) %>"
													 alt="<%= escapeHtml(auctionTitle) %>" class="bid-history-image">
											<%
												} else {
											%>
										...
										<div class="bid-history-product-info">
											<span class="bid-history-auction-number">NO. <%= auctionNo %></span>
											<strong class="bid-history-product-title"><%= escapeHtml(auctionTitle) %></strong>
										</div>
									</a>
								</td>
								
								<td class="bid-history-price-cell my-price">
									<strong><%= priceFormat.format(myHighestBidPrice) %>원</strong>
								</td>

								<td class="bid-history-price-cell current-price">
									<span class="bid-history-price-label"><%= escapeHtml(displayCurrentPriceLabel) %></span>
									<strong data-realtime-current-price><%= priceFormat.format(currentPrice) %>원</strong>
									<span class="bid-history-total-bid-count">전체 입찰 <span data-realtime-bid-count><%= auctionBidCount %></span>건</span>
								</td>
								...
								<td class="bid-history-status-cell">
									<span class="bid-history-status <%= displayAuctionStatusClass %>"><%= escapeHtml(displayAuctionStatus) %></span>
								</td>

								<td class="bid-history-result-cell">
									<div class="bid-history-result-content">
										<span class="bid-history-result <%= displayBidResultClass %>"><%= escapeHtml(displayBidResult) %></span>
										<%
											if ("CANCELED".equals(auctionStatus) && auctionAdminReason != null && !auctionAdminReason.isBlank()) {
										%>
											<span class="bid-history-result-reason" title="<%= escapeHtml(auctionAdminReason) %>"><%= escapeHtml(auctionAdminReason) %></span>
										<%
											}
										%>
									</div>
	...
	function initializeMyPageAuctionRealtime() {
		...
		function animatePriceCountUp(element, endPrice) {
			...
			function drawPriceFrame(currentTime) {
				...
				//초반에는 빠르게 증가하고 마지막 가격 부근에서 부드럽게 감속하는 ease-out 곡선
        ...
        /* =========================
           경매 항목 수집
           ========================= */
        realtimeItemElements.forEach(function (itemElement) {
        ...
        function parseWebSocketMessage(message) {
            ...
            const parsedMessage = {
                type: messageParts[0] || "", auctionNo: null
            };
